...
McJradiation_names = c("Year","Doy","Rn_water_MacJ_U","Rn_water_MacJ_B","Rn_water_MacJ_S","Rs_water_MacJ_U","Rs_water_MacJ_B","Rs_water_MacJ_S","Rlin_MacJ_U","Rlin_MacJ_B","Rlin_MacJ_S","Rlout_MacJ_U","Rlout_MacJ_B","Rlout_MacJ_S","RsMJm2day","Rlin_MacJBrut","Rlin_MacJBrunt","efromTdew","e_air_avg","VP")


ReconstructWB_names = c("Year","Doy","Rain_total","PminusEaero_U","PminusEaero_B","PminusEaero_S","PminusEMcJ_U","PminusEMcJ_B","PminusEMcJ_S","PminusEDBVD_U","PminusEDBVD_B","PminusEDBVD_S","PminusEFH_U","PminusEFH_B","PminusEFH_S","WLchange_U","WLchange_B","WLchange_S","FeedtermEaero_U","FeedtermEaero_B","FeedtermEaero_S","FeedtermEMcJ_U","FeedtermEMcJ_B","FeedtermEMcJ_S","FeedtermEDBVD_U","FeedtermEDBVD_B","FeedtermEDBVD_S","FeedtermEFH_U","FeedtermEFH_B","FeedtermEFH_S","switchOW_U","switchOW_B","switchOW_S","AbsFeedtermDVVD_U","AbFeedtermDVVD_B","AbFeedtermDVVD_S")


meteodata <- read.table("/Users/anneverhoef/Documents/Kazahkstan/Burabaylakescode/Inputfile20132016tocalculate3lakeEBfluxes3.txt",header=TRUE, sep="", col.names=meteo_names)
...
matplot(Lakelevelsdata$deciYear,Lakelevelsdata$FeedtermEaero_U,type="l",lty=1, col=1,ylim = c(-400,800),xlim = c(2013,2017),xlab="Doy",ylab="Cumulative 'loss' term required (mm day-1)")
...
text(2015,550,"Lake Ulken",cex=1.0)

text(2015,450, "Positive represents need for another gain term..",cex=1.0)
text(2015,-250, "Negative represents need for another loss term..",cex=1.0)


legend(2014,800, c("Aerodynamic","McJannet","De Bruin & Van Dool","Finch & Hall"),cex=0.8,lty=1:1:1:1,col=1:2:3:4);
...
text(2015.5,600,"Lake Burabay",cex=1.0)

text(2015.5,800, "Positive represents need for another gain term..",cex=1.0)
...
matplot(Lakelevelsdata$deciYear,Lakelevelsdata$FeedtermEaero_S,type="l",lty=1, col=1,ylim = c(-400,800),xlim = c(2013,2017),xlab="Doy",ylab="Cumulative 'loss' term required (mm day-1)")
...
text(2015.5,600,"Lake Shortandy",cex=1.0)

text(2015.5,800, "Positive represents need for another gain term..",cex=1.0)
text(2015.5,-350, "Negative represents need for another loss term..",cex=1.0)
